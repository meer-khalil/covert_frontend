...
const Blogs = () => {
  ...
  const addBlogToCategory = async (_category, blogId) => {
    ...
    // return
    try {
      const { category } = await api.put(`/categories/addblog/${_category}`, {
        id: blogId,
      });
  ...
  const removeBlogToCategory = async (_category, blogId) => {
    try {
      const res = await api.put(`/categories/removeblog/${_category}`, {
        id: blogId,
      });
            ...
            <>
              <div className="flex gap-5">
                <div className="flex-1 h-28 overflow-hidden bg-cover">
                  <img
                    src={`${process.env.REACT_APP_BACKEND_RESOURCE}/images/blog/${post?.cover?.filename}`}
                    alt="post"
