      ...
      <Link to={`/property-details/${property?.slug}`}>
        <div className=" h-[300px] rounded-tl-xl rounded-tr-xl overflow-hidden">
          <img
            ...
            src={`${process.env.REACT_APP_BACKEND_RESOURCE}/images/${
              property?.images[property?.defaultImage || 0]?.filename
            }`}
            alt="Property Image"
          />
        </div>
        <div className="px-3 py-3">
          <h2 className="text-lg font-semibold mb-2">{property?.address}</h2>
      ...
      {user?.role === "admin" && (
        <>
          <div className="flex justify-between items-center  mx-5 my-3">
            ...
            <div>
              <a
                href={`/admin/property/edit/${property?.slug}`}
                target="_blank"
                className=" mr-3"
              >
          ...
          {user?.role === "admin" && (
            <div className="absolute right-2 -top-9 cursor-pointer text-3xl font-bold flex gap-5">
              <Button
                ...
                variant="contained"
                onClick={() =>
                  handleClick({ showHome: !property.showHome }, property.slug)
                }
              >
              ...
              <Button
                ...
                variant="contained"
                onClick={() =>
                  handleClick({ sold: !property.sold }, property.slug)
                }
              >
