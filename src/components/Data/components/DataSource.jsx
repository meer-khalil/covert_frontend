...
const DataSource = ({ category }) => {
  return (
    <div className="border-2 border-primary px-2 mt-10 mx-2 relative">
      ...
      <h3 className=" text-primary text-2xl font-bold my-3">{category}</h3>
      <div>
        <ul className="list-disc ml-5">
          {dataSource[category]?.list?.map((e, i) => (
            <li key={i}>
              <p className=" text-sm break-words">{e}</p>
            </li>
          ))}
          <br />
          <li>
