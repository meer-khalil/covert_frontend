...
const InvestmentPayback = ({ property }) => {
  ...
  function calcPayback() {
    ...
    let count = 0;
    while (count < 10) {
      let cashflow = (yearlyRent[count] - monthlyExpense * 12) - (yearlyRent[count] * 0.1) - mortgage * 12;
      ...
      <div className=' mt-20 overflow-x-scroll md:overflow-auto'>
        ...
        <table className='w-full' style={{
            ...
            <tr className=' bg-primary'>
              <td className='pl-2 text-xl text-white'>Years</td>
              {
                (new Array(10).fill(0)).map((e, index) => (
                  <td key={index} className='py-2 text-xl text-white'>{2023 + index}</td>
                ))
              }
            </tr>
          </thead>
          ...
          <tbody>
            ...
            <tr>
              <td className=' text-primary font-bold'>Initial Cash Investment</td>
