...
const MarketAnalysis = ({ zipCode, data, capData }) => {
  ...
  function calculateAverageCapRate(capRates) {
    ...
    const sumOfCapRates = capRates.reduce((total, capRate) => total + capRate, 0);
          ...
          [
            {
              title: 'Rent Growth',
              property: [
                { title: 'Average Rent Growth', value: rentGrowth }
              ],
            ...
            },
            {
              title: 'CAP Rate',
              property: [
                { title: 'Cap Rate', value: capAverage }
              ],
              link: `/data/${zipCode}/Cap Rate`
            }
          ].map((el, i) => (
            <div className={`rounded-lg bg-gray-200 relative p-5 mb-9 pt-28 px-10 ${i !== 0 ? "mt-20" : ""}`}>
              {
              ...
              <div class="relative mt-5">
                <input type="range" min="0" max="100" value="50" class="slider w-full accent-primary" />
              </div>
              <div className='flex justify-center gap-5 mt-10'>
                {/* <p>Historical Average</p> */}
                <Link to={el.link}>
                  <p className=' text-lg font-semibold text-primary'>View Charts</p>
                </Link>
              </div>
